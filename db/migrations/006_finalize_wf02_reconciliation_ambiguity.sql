...
CREATE OR REPLACE FUNCTION finalize_wf02_reconciliation_ambiguity(
    p_operation_id uuid,
    p_case_id uuid,
    p_token_id uuid,
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'match_count must be at least two';
    END IF;
...
COMMENT ON FUNCTION finalize_wf02_reconciliation_ambiguity(
    uuid,
    uuid,
    uuid,
    text,
...
IS
'Atomically finalizes an ambiguous WF02 Gmail reconciliation result.';
